...
x = Conv2D(filters=128, kernel_size=3, strides=(1,1), padding="same", activation="relu")(model_input)
x = BatchNormalization()(x)

x = Conv2D(filters=128, kernel_size=3, strides=(1,1), padding="same", activation="relu")(x)  # kernel_size is window size, check alphago window_size
x = BatchNormalization()(x)
x = MaxPooling2D()(x)

x = Conv2D(filters=196, kernel_size=3, strides=(1,1), padding="same", activation="relu")(x)
x = BatchNormalization()(x)
...
x = Flatten()(x)
x = Dense(128, activation="relu")(x)   # play with this
x = Dropout(0.4)(x)
...
for k in range(51):
    ...
    print(f"train_location: {train_location}")
    val_filename = val_shell.format(n)
    val_location = DATA_PATH + val_filename
    print(f"val_location: {val_location}")
